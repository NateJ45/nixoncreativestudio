...
- `HeroShowcase.astro` — the portfolio cascade in the hero's right column (at `lg` and up): three staggered browser-chrome frames, each crossfading real case-study cover screenshots with the live host in its address bar. Collection-driven (featured first, then newest others, pooled from up to six covers); each frame starts on a different site and a single round-robin timer advances one frame at a time, so only one crossfade happens at once and no two frames ever show the same site. Entirely `aria-hidden` (decorative; the real navigation is the "See the work" CTA beside it and the Selected Work section below), reduced-motion safe (each frame holds a distinct shot), and pauses when the tab is hidden.
