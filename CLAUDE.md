...
- `MobileNav.tsx` — the small-viewport navigation: a full-screen navy panel (a shadcn Sheet underneath, so focus-trap, Escape, scroll-lock, and dialog ARIA all stay intact). Oversized Bebas nav links, each with a short honest descriptor, hairline-divided like an editorial index, with a staggered entrance on open. Navy in both themes by choice: it is a full-screen overlay menu, kept dark even in light mode so it reads as a focused layer over the page rather than part of it. Carries an amber "Start a project" CTA and a "Get in touch" block (email, phone, inline Instagram / LinkedIn glyphs, and the theme toggle). The active route shows in amber with a persistent arrow. Below md (768px); `client:only="react"`.
